...
// 크기: 스프라이트 최대 640px(화면 표시 ≤140css px ×3배 = 420px 로 충분), 배경 플레이트 최대 1200px. WebP q82.
...
for (const [key, a] of Object.entries(manifest.assets)) {
  ...
  const max = key === 'plate' ? 1200 : 640;
  const out = await sharp(buf).resize({ width: max, height: max, fit: 'inside', withoutEnlargement: true }).webp({ quality: 82, alphaQuality: 90 }).toBuffer();
